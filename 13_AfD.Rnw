...
monthly.avg %>% filter(party=="CDU.CSU") %>% select(datum, month.avg) %>% arrange(datum) %>% select(month.avg)-> month.CDU.CSU
...
month.CDU.CSU.SMA3 <- SMA(month.CDU.CSU.ts, n=8)
plot.ts(month.CDU.CSU.SMA3)
@
...
\subsubsection{If there is no trend and no seasonality: HoltWinters with $\beta$=0 and $\gamma$=0}
<<label=Forecast1,fig=TRUE,include=FALSE, echo=FALSE>>=
CDU.CSU.forecast <- HoltWinters(month.CDU.CSU.ts, beta=FALSE, gamma=FALSE)
plot(CDU.CSU.forecast)
@

\begin{center}
\includegraphics[width=0.5\textwidth]{AfD-Forecast1}
\captionof{figure}{Forecast based on CDU.CSU polls 2013}
\label{fig:AfD-Forecast1}
\end{center}

This simply follows the data with some delay.
