...
\textbf{Conclusion}: the HoltWinters prediction based on monthly average of all the polls from 2009-2013 give 95\%-confidence intervals that ALL contain the actual result.  The naive method, based on the average of the last month of polling before the election gives worse confidence intervals. The HoltWintere prediction based on the monthly average of the polls in the election year (2103) also give worse confidence intervals. However: the interal lengths are different and considerably larger in the HoltWinters(all) model. When we look at the actual error of the prediction and take the SSE as a measure Naive wins overall, HoltWinters(all) is a close second, while HoltWinters(2013) a distant third. Maybe a simple linear model?
