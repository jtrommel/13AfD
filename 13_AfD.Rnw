...
<<label=AfD4,fig=TRUE,include=FALSE, echo=TRUE>>=
ggplot(data=poll.results,aes(x=Fieldwork.date)) + 
  geom_point(aes(y=AfD),colour="black") +
  stat_smooth(aes(y=AfD))
@
...
<<label=PollingFirm1,fig=TRUE,include=FALSE, echo=TRUE>>=
poll.results.13.g %>% 
...
<<label=PollingFirm2,fig=TRUE,include=FALSE, echo=TRUE>>=
poll.results.13.g %>% 
...
<<label=SampleSize,fig=TRUE,include=FALSE, echo=TRUE>>=
...
<<label=AfD5,fig=TRUE,include=FALSE, echo=TRUE>>=
