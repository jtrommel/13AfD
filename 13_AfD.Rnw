...
mytable.pred <- data.frame(party=c("O"), result=0, predict=0, lo95=0, hi95=0, low95=0, naive=0, high95=0, stringsAsFactors=FALSE)
for (i in (1:7)) {
  if (i==1) {k <- 1}
  if (i==2) {k <- 3}
  if (i==3) {k <- 5}
  ...
  mytable.pred[i,6:8] <- CI95(as.vector(t(last.month[2])))
}
  print(mytable.param)
...
for (i in (1:8)) {
  ...
  mytable.pred[i,3] <- ifelse(i != 7, pred[1,1], c(0))
  mytable.pred[i,4] <- ifelse(i != 7, pred[1,2], c(0))
  mytable.pred[i,5] <- ifelse(i != 7, pred[1,3], c(0))
  mytable.pred[i,6] <- ifelse(election.result[i+2] < pred[1,2] & election.result[i+2] > pred[1,3], "Yes", "No")
}
  mytable.pred
@

\textbf{Conclusion}: the HoltWinters prediction based on the polls give 95\%-confidence intervals that contain the actual result.  The naive method, based on the average of the last month of polling before the election gives worse confidence intervals. Maybe a simple linear model?
...
No: the HoltWinters model based on the monthly average over the whole period is the best!
...
<<label=AfD3,fig=TRUE,include=FALSE, echo=TRUE>>=
ggplot(data=poll.results,aes(x=Fieldwork.date)) + 
  geom_point(aes(y=AfD),colour="black") +
  stat_smooth(aes(y=AfD))
@
